    ...
    def fit(self, X, y=None):
        ...
        for epoch in range(self.n_epochs):
            ...
            with torch.no_grad():
                ...
                train_auc.append(np.round(train_score, 6))

                out = self.network(val_dist)
                val_score = roc_auc_score(val_y.cpu(), out.cpu())
                val_auc.append(np.round(val_score, 6))
                ...
                # save best model
                if (train_score > best_val_score)&(abs(train_score-train_auc[-1])<0.001)&(len(best_dict)==0):
